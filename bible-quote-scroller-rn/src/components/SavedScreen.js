...
import { getSavedQuotes, toggleSave } from "../services/SaveService";
...
export default function SavedScreen() {
  ...
  const handleRemove = async (verse) => {
    await toggleSave(GUEST_ID, verse.id);
    load(); // refresh list
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <TouchableOpacity onPress={() => handleRemove(item)} activeOpacity={0.7}>
        <Ionicons name="bookmark" size={22} color={COLORS.accentPlatinum} />
      </TouchableOpacity>
    </View>
  );
