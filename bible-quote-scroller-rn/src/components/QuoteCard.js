...
import { isSaved, toggleSave } from "../services/SaveService";
...
export default function QuoteCard({ verse }) {
    ...
    useEffect(() => {
        ...
        isSaved(verse.id).then(setSaved);
    }, [verse.id]);
    ...
    const handleLike = async () => {
        const newLiked = await toggleLike(GUEST_ID, verse.id);
        setLiked(newLiked);
        ...
        <TouchableWithoutFeedback onPress={handlePress}>
            <View style={styles.cardContainer}>
                ...
                <View style={styles.actionButtons}>
                    ...
                    <TouchableOpacity
                        onPress={async () => {
                            const newSaved = await toggleSave(GUEST_ID, verse.id);
                            setSaved(newSaved);
                            Animated.sequence([
                    ...
                    >
                        ...
                        >
                            <Ionicons
                                ...
                                size={34}
                                color={saved ? COLORS.accentPlatinum : "rgba(255,255,255,0.75)"}
                            />
                        </Animated.View>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    bookmarkGlow: {
        shadowColor: COLORS.accentPlatinum,
        shadowOffset: { width: 0, height: 0 },
        shadowOpacity: 0.6,
        shadowRadius: 8,
        elevation: 6,
